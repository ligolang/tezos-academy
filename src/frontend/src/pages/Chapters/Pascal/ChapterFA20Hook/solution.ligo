...
{
  function convert_get(const tm : transfer_destination_descriptor) : option(address) is { 
    const t : transfer_destination_descriptor_ = Layout.convert_from_right_comb((tm:transfer_destination_descriptor)) 
  } with t.to_;

  function get_receiver(const txm : transfer_descriptor) : list(option(address)) is {
  ...
  // Type your solution below
    {
      const pm : transfer_descriptor_param = Layout.convert_to_right_comb (p);
      const op : operation = Tezos.transaction(pm, 0mutez, h);
    } with op # ops;
  ...
  function verify_receiver_in_whitelist(const ops : list(operation); const r : address; const wl : set(address)) : list(operation) is
    ...
    if Set.mem (r, wl)
    then ops
    else (failwith ("Not in whitelist") : list(operation));
